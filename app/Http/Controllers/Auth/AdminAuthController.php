<?php
	  ...
	  class AdminAuthController extends Controller
	  {
			  ...
			  */
			 private function registerAdmin(AdminRegistrationRequest $request, bool $isSuperAdmin = false): JsonResponse
			 {
					try {
						  ...
						  $validated['photo'] = $request->hasFile('photo')
								? Storage::disk('public')->url($request->file('photo')->store('admin_images', 'public'))
						  ...
						  $errorMessage = config('app.debug') ? "Server error: {$e->getMessage()}" : 'Server error: Something went wrong.';
			  ...
			  */
			 public function superAdminSignUp(AdminRegistrationRequest $request): JsonResponse
			 {
			 ...
			 public function verifyEmail(VerifyEmailRequest $request): JsonResponse
			 {
					try {
						  ...
						  if (!$admin) {
								 return responseJson(404, 'Admin not found', 'Please check your email or register first.');
						  }
			 ...
			 public function approve(int $pendingAdmin): JsonResponse
			 {
					try {
						  ...
						  if ($admin->is_approved) {
								 return responseJson(403, 'Error', 'This account is already approved.');
						  }
			 ...
			 public function createSubAdmin(CreateSubAdminRequest $request): JsonResponse
			 {
					try {
						  ...
						  if (empty($admin->company_id)) {
								 return responseJson(403, 'Forbidden', 'You must create a company before adding sub-admins.');
						  }
						  
						  $subAdminCount = Cache::remember("sub_admin_count_{$admin->company_id}", 60, function () use ($admin) {
								 return Admin::where('company_id', $admin->company_id)
									  ->where('id', '!=', $admin->id)
									  ->count();
						  });
						  
						  if ($subAdminCount >= 8) {
								 return responseJson(403, 'Forbidden', 'You can only create up to 8 sub-admins.');
						  }
						  
						  $password = Str::password(12, letters: true, numbers: true,  symbols: true);
						  $subAdmin = null;
						  
						  DB::transaction(function () use ($validated, $admin, &$subAdmin, $password) {
			 ...
			 public function login(AdminLoginRequest $request): JsonResponse
			 {
					try {
						  ...
						  if (!$admin->hasVerifiedEmail()) {
								 auth('admin')->logout();
								 return responseJson(403, 'Forbidden', 'Please verify your email address before logging in.');
						  }
						  
						  if (!$admin->is_approved) {
								 auth('admin')->logout();
								 return responseJson(403, 'Forbidden', 'Account pending approval. Please contact the administrator.');
						  }
			 ...
			 public function logout(): JsonResponse
			 {
					try {
						  ...
						  return responseJson(500, 'Server error', config('app.debug') ? $e->getMessage() : 'Logout failed due to server error.');
					}
			 }
			 ...
			 public function requestPasswordReset(RequestPasswordResetRequest $request): JsonResponse
			 {
					try {
						  ...
						  $errorMessage = config('app.debug') ? "Server error: {$e->getMessage()}" : 'Password reset request failed. Please try again later.';
						  return responseJson(500, 'Server error', $errorMessage);
					}
			 }
			 ...
			 public function newPassword(NewPasswordRequest $request): JsonResponse
			 {
					try {
						  ...
						  if (!$this->pinService->verifyPin($admin, $validated['pinCode'], 'reset')) {
								 return responseJson(401, 'Unauthorized', 'Expired PIN. Request a new PIN.');
						  }
